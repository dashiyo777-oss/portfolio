...
### データ系
- [ ] `[ ]` 付き43件（参議院）の正式名称・政党確定
- [ ] `情報不足` 27件のうち調査可能なものを評価
- [ ] `Wikipedia基礎評価` 85件の本格評価

### 機能系
- [ ] Google Search Console 登録（手動作業・無料）
- [ ] 更新告知自動化（GitHub Actions + X API 連携）
- [ ] サイトマップ（sitemap.xml）作成

### `[ ]` 付きエントリについて
全43件が参議院・`party:"無所属"` 表記。正式な氏名・所属政党が未確定で仮登録された候補者。
例: `[福島 瑞穂]`→社民党、`[齊藤 蓮舫]`→立憲民主党 など。
調査後は `[ ]` を外し party を正しく設定すること。
